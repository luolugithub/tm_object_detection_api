...
class CookingThread(threading.Thread):
  ...
  def run(self):
    ...
    distribution = {
        ...
        4: (np.array([0,0,0,20,0,0]), np.array([19,0,0,1,0,0]), np.array([20,0,0,0,0,0,0])),
        5: (model_5, model_6, model_3_1),
        6: (np.array([0,0,20,0,0,0]), np.array([0,0,20,0,0,0]), np.array([0,0,20,0,0,0])),
    }
    ...
    start_pos = 0
    end_pos = window_size
    self.current_state = state[self.previous_state_num]
    # current_distribution = distribution[self.current_state_num][self.current_substate_num]
    first_state = {start_pos: self.previous_state_num}
    ...
    if self.current_state == state[6]:
      # continue
      pass
    else:
      if self.previous_state_num < 2:
        print('current_substate_num', self.previous_substate_num)
        current_distribution = distribution[self.previous_state_num][self.previous_substate_num]
        next_distribution = distribution[self.previous_state_num][self.previous_substate_num + 1]
        q = self.array_to_histogram(self.previous_observation, _COOKING_NUM_CLASSES)
        current_kl = self.kl_divergence(q, current_distribution)
        next_kl = self.kl_divergence(q, next_distribution)
        print('================')
        print(start_pos)
        ...
        if current_kl > next_kl:
          self.current_substate_num = self.previous_substate_num + 1
          self.current_substate_num += 1
          print('################ distribution num change {} -> {} ################'
                .format(self.previous_substate_num, self.current_substate_num))
          print('current_substate_num', self.current_substate_num)
          self.previous_substate_num = self.current_substate_num
        else:
          pass
      else:
        print('################ state change ################')
        current_state = state[self.previous_state_num + 1]
        self.current_state_num = self.previous_state_num + 1
        self.current_substate_num = 0
        print(start_pos)
        print('################ state change {} {} ################'
              .format(current_state, self.current_substate_num))
        print('current_substate_num', current_substate_num)
        # continue
        self.previous_state_num =  self.current_state_num
        pass
    ...
    # self.current_predictions_2 = self.current_state_num
    
    # if self.previous_predictions_2 + 1 == self.current_predictions_2:
    self.bbox2_instance = str(self.previous_state_num) + ' : ' + str(state[self.previous_state_num])
    # if self.previous_predictions_2 != self.current_predictions_2:
    #   self.current_state_num = self.current_predictions_2
    #   print('change')
    #   print('*'*15 + 'gyoza query' + '*'*15)
    #   # self.bbox2_instance = str(self.status) + ' : ' + str(self.category_map[x.argmax()])
    #   self.bbox2_instance = str(current_state_num) + ' : ' + str(state[current_state_num])
    ...
    # # else:
    # #   pass
    #   print('current status: %s %s ' % (self.current_state_num, self.category_map[self.current_state_num]))
  ...
  with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
      ...
      while(True):
        ...
        #               center1_width-threshold-440:center1_width+threshold-440]
        bbox2 = frame[center_height-threshold+190:center_height+threshold+190,
                      center1_width-threshold-240:center1_width+threshold-240]        
        ...
        print('current_state_num', current_state_num)
        print('current_substate_num', current_substate_num)        
        if count % 100 == 31:
          ...
          print('thread2.current_state_num', thread2.previous_state_num)
          print('thread2.current_substate_num', thread2.previous_state_num)
          previous_state_num = thread2.previous_state_num
          previous_substate_num = thread2.previous_state_num
